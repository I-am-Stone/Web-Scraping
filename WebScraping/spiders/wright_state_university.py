...
class WrightSpider(scrapy.Spider):
    ...
    def parse(self, response):
        courses = response.xpath(
            '//div[contains(@class,"item-list")]//a/@href'
        ).getall()
    ...
    def parse1(self, response):
        ...
        course_web_url = response.xpath(
            '//li[contains(@class,"department")]/a/@href'
        ).get()
        if course_web_url:
    ...
    def parse_courses(self, response):
        ...
        course_des = response.xpath(
            '(//h2[contains(text(),"Why")]//following-sibling::p)[position() < 3]'
        ).getall()
        carrer = response.xpath(
            '//h4[contains(text(),"work as")]//following-sibling::ul'
        ).get()
