...
class DekotaSpider(scrapy.Spider):
    ...
    def parse1(self, response):
        ...
        intake = response.xpath(
            '//h3[contains(text(),"Start Terms")]/parent::div'
        ).get()
        carrer = response.xpath(
            '//h2[contains(text(),"Career possibilities")]/parent::div//following-sibling::div//ul'
        ).get()
        study_mode = response.xpath(
            '//h3[contains(text(),"Available")]/parent::div'
        ).get()
        course_des = response.xpath(
            '(//div[contains(@class,"main__content")])[1]'
        ).get()
