...
class DurationProcessor:
    ...
    def clean_duration(value: Optional[str]) -> Optional[str]:
        ...
        value = (
            value.replace("2025 tuition fee", "")
            .replace("120 points", "")
            .replace("180 points", "")
            .replace("140 points", "")
        )
...
class WebscrapingItem(scrapy.Item):
    ...
    Course_Description = scrapy.Field(
        input_processor=MapCompose(TextCleaner.clean_html), output_processor=TakeFirst()
    )

    # Location and career info
    Career = scrapy.Field(
        input_processor=MapCompose(TextCleaner.clean_base), output_processor=TakeFirst()
    )
    City = scrapy.Field(
        input_processor=MapCompose(
            TextCleaner.clean_html, lambda x: x.replace("Distance Learning", "")
        ),
        output_processor=TakeFirst(),
    )
    ...
    Duration = scrapy.Field(
        input_processor=MapCompose(
            TextCleaner.clean_html, DurationProcessor.clean_duration
        ),
        output_processor=TakeFirst(),
    )
    Duration_Term = scrapy.Field(
        input_processor=MapCompose(
            ...
            TextCleaner.remove_numbers,
            DurationProcessor.process_term,
        ),
        output_processor=TakeFirst(),
    )
